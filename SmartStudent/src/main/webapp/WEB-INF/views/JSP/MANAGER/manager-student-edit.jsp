...
<body>
	<h1>Edit Student</h1>
	<form
		action="${pageContext.request.contextPath}/manager/students/${student.studentId}/edit"
		method="post">
		...
		<p>
			Street: <input type="text" name="street"
				value="${student.address.street}" required />
		</p>
		<p>
			City: <input type="text" name="city" value="${student.address.city}"
				required />
		</p>
		<p>
			State: <input type="text" name="state"
				value="${student.address.state}" required />
		</p>
		<p>
			Country: <input type="text" name="country"
				value="${student.address.country}" required />
		</p>
		<p>
			Zip Code: <input type="text" name="zipCode"
				value="${student.address.zipCode}" required />
		</p>

		<h2>Division Information</h2>
		<p>
			Division: <select name="division.divisionId" required>
				<c:forEach var="division" items="${divisions}">
					<option value="${division.divisionId}"
						${division.divisionId == student.division.divisionId ? 'selected' : ''}>${division.name}</option>
				</c:forEach>
			</select>
		</p>
		...
		<p>
			Contact Number: <input type="text" name="contactNumber"
				value="${student.contactNumber}" required />
		</p>
		<p>
			Guardian Name: <input type="text" name="guardianName"
				value="${student.guardianName}" required />
		</p>
		<p>
			Guardian Contact: <input type="text" name="guardianContact"
				value="${student.guardianContact}" required />
		</p>
		<p>
		...
		<p>
			Enrollment Date: <input type="date" name="enrollmentDate"
